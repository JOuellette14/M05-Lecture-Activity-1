...
int main()
{
    ...
    drinkOrder dorder;
    dorder.addDrink(*myDrink);
    drink newDrink(TEA, ICE, LARGE, "", "Sweet");
    drink regularDrink(TEA, HOT, MED, "", "");
    regularDrink = newDrink;
    dorder.addDrink(newDrink);
    drinkOrder dorder2(dorder);

    dorder.addDrink(regularDrink);

    std::cout << dorder.tostring() << std::endl;
    std::cout << dorder2.tostring() << std::endl;
